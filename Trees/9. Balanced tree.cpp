...
class Solution{
    ...
    pair<int,bool> solve(Node *root){
        ...
        bool val_height= abs(left_part.first-right_part.first)<=1? true:false;
        int height=max(left_part.first,right_part.first)+1;
        pair<int,bool> ans=make_pair(height,true);
        if(left_part.second && right_part.second && val_height){
            ans.second=true;
        }
        else{
            ans.second=false;
